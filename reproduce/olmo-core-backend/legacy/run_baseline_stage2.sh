...
STAGE=stage2
RUN_NAME=olmo3-1b-baseline-5e-3
...
torchrun \
  ...
  cfgs/OLMo3-1B-stage2-baseline.py \
  --name ${RUN_NAME} \
  --data-root=${DATA_DIR} \
  --save-folder=${CKPT_DIR}/${RUN_NAME}/${STAGE} \
  --work-dir ${WORK_DIR} \
  --trainer.load_path= /mnt/shared-storage-gpfs2/intern-pretrain-shared02/taoji/ckpts/Olmo3-MoSE/olmo3-1b-baseline-5e-3/stage1/step75080\
