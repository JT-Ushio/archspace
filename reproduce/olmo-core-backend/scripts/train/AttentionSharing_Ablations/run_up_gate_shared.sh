...
RUN_NAME="${RUN_NAME:-olmo3-1b-up-gate-shared}"
STAGE="${STAGE:-stage1}"
...
exec "${OLMO_ENV}/bin/python" -m torch.distributed.run \
  ...
  cfps/BS256_Seq8K_30B_WSD_FP8/AttentionSharing_Ablations/up_gate_shared.py \
  --name "${RUN_NAME}" \
  ...
  --model.block.sequence_mixer.share_scope=none \
