...
def get_cubit_cli_parser() -> argparse.ArgumentParser:
    parser = get_cli_parser()
    parser.add_argument(
        "--cubit-start-layer",
        type=int,
        default=0,
        help="Zero-based first transformer layer that uses Cubit (default: 0).",
    )
    parser.add_argument(
        "--cubit-end-layer",
        type=int,
        default=None,
        help="Exclusive end of the Cubit layer range (default: number of layers).",
    )
    return parser
    ...
    *,
    ...
    if sequence_length <= 0 or max_target_sequence_length % sequence_length != 0:
        raise OLMoConfigurationError(
            "sequence_length must be positive and divide max(1024, sequence_length)"
        )
    if GLOBAL_BATCH_SIZE % rank_microbatch_size != 0:
        raise OLMoConfigurationError(
            "global batch size must be divisible by "
            f"{RANK_MICROBATCH_SEQUENCES} * sequence_length"
        )
